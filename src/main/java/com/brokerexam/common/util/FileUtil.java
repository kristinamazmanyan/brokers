...
public class FileUtil {
    ...
    public static void write(File file, List<String> records)
            throws IOException {
        write(file, records, false);
    }

    public static void write(File file, List<String> records, boolean append)
            throws IOException {
        mkdirs(file);
        BufferedWriter writer = null;
    ...
    public static void copy(File source, File destination, boolean overwrite)
        ...
        if (destination.exists() && !overwrite) {
            throw new IOException("Copying file " + source.getPath() + " to "
    ...
    public static void move(File source, File destination, boolean overwrite)
        ...
        if (destination.exists()) {
            if (overwrite) {
                ...
                throw new IOException("Moving file " + source.getPath()
                        + " to " + destination.getPath() + " failed."
                        + " The destination file already exists.");
            }
        }
        ...
        if (!source.renameTo(destination)) {
            throw new IOException("Moving file " + source.getPath() + " to "
                    + destination.getPath() + " failed.");
        }
    }
