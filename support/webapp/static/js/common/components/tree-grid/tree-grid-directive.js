    ...
    .run(
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          " <table class=\"table table-hover tree-grid\">\n" +
          "   <thead>\n" +
          "     <tr style=\"background-color:#E0D1C2\" >\n" +
          //"       <th><a ng-if=\"expandingProperty.sortable\" ng-click=\"sortBy(expandingProperty)\">
          // {{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</a><span
          // ng-if=\"!expandingProperty.sortable\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</span>
          // <i ng-if=\"expandingProperty.sorted\" class=\"{{expandingProperty.sortingIcon}} pull-right\"></i></th>\n" +
          "       <th ng-repeat=\"col in colDefinitions\"><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">" +
          "{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span>" +
          "<i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
          "     </tr>\n" +
          "   </thead>\n" +
          "   <tbody>\n" +
          ...
          "       </td>\n" +
          "       <td ng-repeat=\"col in colDefinitions.slice(1,colDefinitions.length)\">\n" +
      ...
      function ($timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            ...
            attrs.iconExpand = attrs.iconExpand ? attrs.iconExpand : 'glyphicon glyphicon-triangle-right';
            attrs.iconCollapse = attrs.iconCollapse ? attrs.iconCollapse : 'glyphicon glyphicon-triangle-bottom';
            attrs.iconLeaf = attrs.iconLeaf ? attrs.iconLeaf : 'glyphicon glyphicon-option-vertical';
            ...
            scope.getClass = function (row) {
                console.log( " 'level-'" +  row.level + (row.branch.selected ? ' active':''));
                return " level-"+  row.level + (row.branch.selected ? ' active':'');
